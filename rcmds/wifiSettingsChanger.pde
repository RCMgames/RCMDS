...
void setupWifiSettingsChanger(float x, float y) {
  ...
  redRecallWifiButton=new Button(x-width/12, y+height/6.5, height/12, color(150, 0, 0), color(200), null, 0, true, false, "recall");
  ...
  blueRecallWifiButton=new Button(x+width/12, y+height/6.5, height/12, color(0, 0, 150), color(200), null, 0, true, false, "recall");
...
void runWifiSettingsChanger() {
  ...
  if (redSaveWifiButton.justPressed()) {
    ...
    saveStrings("REDwifiSettings.txt", settings);
  }
  if (redRecallWifiButton.justPressed()) {
    try {
      String[] settings=loadStrings("REDwifiSettings.txt");
      wifiIP=settings[0];
  ...
  if (blueSaveWifiButton.justPressed()) {
    String[] settings={wifiIP, str(wifiPort)};
    saveStrings("BLUEwifiSettings.txt", settings);
  }
  if (blueRecallWifiButton.justPressed()) {
    try {
      String[] settings=loadStrings("BLUEwifiSettings.txt");
      wifiIP=settings[0];
      wifiPort=int(settings[1]);
    }
    catch(Exception e) {
